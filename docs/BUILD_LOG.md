...
## 2026-08-22 — Phase 5.5 pre-live readiness audit

- Ran a scope-locked audit (no product expansion) across the full data path, C++ exchange core, frontend, and ops/security/gates, using four parallel read-only reviews each required to cite `file:line` and separately verify correct behavior, not just report problems. Findings, test matrix, and readiness scorecard are recorded in `docs/audits/pre-live-readiness.md` and `docs/audits/pre-live-final-report.md`; a units-consistency reference is in `docs/audits/unit-consistency.md`; a new operator runbook is `docs/operations/live-shadow-runbook.md`.
- Fixed all seven P0 findings: the frontend's "LIVE FEED" label conflated WebSocket connection state with data authenticity (now split into separate STREAM/DATA facts, and the synthetic market tick engine always honestly stamps `dataMode: "synthetic"` regardless of the worker's ingestion mode); the shipped `docker compose --profile live-shadow` config defaulted `CULT_CASCADE_HASH_SECRET` to a committed, publicly-known string (now fails the compose invocation outright if unset); `AttributionStore` silently degraded to a random per-process key instead of failing closed when a durable deployment had no real secret (now throws); `CompositeSink` could lose an aggregate window from both PostgreSQL and its local JSONL durability backstop on a single Postgres failure (sinks are now attempted independently); `PostgresAggregateSink`'s single `pg.Client` had no error handler and would crash on an unhandled connection-error event (now logs structured diagnostics and exits deliberately); the worker's `STALE` source-health transition never reached the database unless an aggregate window happened to roll over or the socket closed, so a silently-dead stream could leave dashboards reporting `HEALTHY` indefinitely (now flushes a health-only snapshot on the transition); and `shutdown()` never closed the active Jetstream socket, risking a SIGTERM hang past a container's grace period (now does).
- Found and fixed a genuine C++ order-book correctness bug, empirically reproduced before fixing: `LimitOrderBook::available()`'s `fill_or_kill` precheck counted contra-side liquidity that sat behind the requester's own resting order, which the real matching loop can never reach once self-trade prevention interrupts it under `cancel_newest`/`cancel_both` — a FOK order could pass its precheck and then partially fill, violating FOK's all-or-nothing contract. Fixed by making the precheck mirror the matching loop's actual per-STP-mode behavior, with a new regression test in `cpp/tests/test_main.cpp`.
- Fixed a real performance bug: `EventDeduplicator.seen()` scanned its entire map on every event to prune expired entries — O(n) per call, effectively O(n²) over an hour of sustained traffic once the map holds real Jetstream volume. Benchmarked: the old implementation did not finish 200,000 simulated events in under two minutes; the fixed version (amortized O(1), exploiting that Map insertion order tracks expiry order under non-decreasing timestamps) processed 500,000 in 329ms.
- Also fixed: `forgetPost()` (DELETE handling) now issues a durable tombstone instead of only clearing the in-memory cache, so a warm-started worker cannot resurrect attribution for a post it was explicitly told to forget; found and regenerated three stale Phase 4 replay fixtures left over from the prior session's price-scale fix (`npm run replay:hft` was failing a hash check against them); added a global frontend `ErrorBoundary` (keyed by route so it resets cleanly on navigation) plus optional chaining on the specific market-critical fields that could previously white-screen a whole page on a missing API field.
- Documented, deliberately left unfixed as out of this pass's minimal-bug-fix scope (each needs a real interface/schema change or a product judgment call): the pre-trade risk price collar is a structural no-op for market orders in both the C++ and TypeScript risk checks; six of eight `RiskDecision` branches have no dedicated test; `evaluateAlerts()` exists with concrete thresholds but is wired into no running process; the API does not enforce `DATABASE_URL` under live modes the way the worker does. Full list with severities in `docs/audits/pre-live-readiness.md`.
- Verification this pass: `npm test` 52/52, lint/typecheck/build clean, a clean C++ rebuild passed all three CTest programs including the new regression test, `npm run test:differential` passed (25 metrics), `run_great_cry_shock` hashed identically across two runs, and a single manual, non-persistent Bluesky Jetstream connectivity smoke (`npm run smoke:live`) succeeded against the real public endpoint. PostgreSQL-dependent and ASan/UBSan verification were not run locally (Docker unavailable, no local clang — consistent with every earlier phase); CI is relied on for both, as this project has always done.
- Final decision: **READY FOR LIVE-SHADOW**, conditioned on this commit's CI passing (PostgreSQL + sanitizer jobs), setting a real `CULT_CASCADE_HASH_SECRET`, and following the new runbook's first-hour checklist since several fixes are unit-verified but not yet live-drilled. `live-market` remains gated and unaffected by this pass.
